...
SELECT
	...
	, cc.DateRecorded
	, cc.DailyCases
    -- , sum(cc.DailyCases) over(order by cc.DateRecorded) AS CumulativeCases      -- cummulative cases UK-wide
	, sum(cc.DailyCases) over(partition by cc.Country order by cc.DateRecorded) AS CumulativeCases
    -- moving weekly average (assuming prev 6 days always have data):
    , sum(cc.DailyCases) over(partition by cc.Country order by cc.DateRecorded rows between 6 preceding and current row) / 7 AS MovingWeeklyAvgCases
FROM
    CovidCase cc
